<cfcomponent extends="algid.inc.resource.base.service" output="false">
		...
		<cfelse>
			...
			<cfquery datasource="#variables.datasource.name#" result="results">
				...
				(
					...
					B'#(arguments.tag.getIsPluginOnly() ? 1 : 0)#'
				)
			</cfquery>
			...
			<cfquery name="results" datasource="#variables.datasource.name#">
				...
				WHERE tag = <cfqueryparam cfsqltype="cf_sql_varchar" value="#arguments.tag.getTag()#" />,
					AND "isPluginOnly" = B'#(arguments.tag.getIsPluginOnly() ? 1 : 0)#'
			</cfquery>
	...
	<cffunction name="readTag" access="public" returntype="component" output="false">
		...
		<cfset tag = createObject('component', 'plugins.tagger.inc.model.modTag').init(variables.i18n, variables.locale) />
	...
	<cffunction name="readTags" access="public" returntype="query" output="false">
		...
		<cfquery name="results" datasource="#variables.datasource.name#">
			...
			<cfif structKeyExists(arguments.filter, 'isPluginOnly')>
				AND "isPluginOnly" = B'#(arguments.filter.isPluginOnly ? 1 : 0)#'
			</cfif>
